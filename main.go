...
func request(fullurl string, printStatus bool) string {
    ...
    if printStatus {
        ...
        fmt.Printf("[Linkfinder] %s : %s%d%s\n", fullurl, statusColor, resp.StatusCode, colorReset)
    }
...
func regexGrep(content string, baseUrl string, patterns []patternDef) {
    for _, p := range patterns {
        ...
        for _, v := range matches {
            ...
            fmt.Printf("[+] Found [%s%s%s] [%s%s%s] [%s]\n", confidenceColor, p.Name, colorReset, colorBlue, v, colorReset, baseUrl)
        }
    }
}

func linkFinder(content, baseURL string, completeURL, printStatus bool) {
    linkRegex := `(?:"|')(((?:[a-zA-Z]{1,10}://|//)[^"'/]{1,}\.[a-zA-Z]{2,}[^"']{0,})|((?:/|\.\./|\./)[^"'><,;| *()(%%$^/\\\[\]][^"'><,;|()]{1,})|([a-zA-Z0-9_\-/]{1,}/[a-zA-Z00_\-/]{1,}\.(?:[a-zA-Z]{1,4}|action)(?:[\?|#][^"|']{0,}|))|([a-zA-Z0-9_\-/]{1,}/[a-zA-Z0-9_\-/]{3,}(?:[\?|#][^"|']{0,}|))|([a-zA-Z0-9_\-]{1,}\.(?:php|asp|aspx|jsp|json|action|html|js|txt|xml)(?:[\?|#][^"|']{0,}|)))(?:"|')`
    ...
    for _, match := range matches {
        ...
        if printStatus {
            ...
            fmt.Printf("[+] Found link: [%s%s%s] in [%s] \n", colorBlue, link.String(), colorReset, base.String())
        }
    }
}
